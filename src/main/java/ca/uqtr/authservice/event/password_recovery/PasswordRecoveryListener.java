...
public class PasswordRecoveryListener implements
    ...
    public void onApplicationEvent(OnPasswordRecoveryEvent event) {
        if (mailService.equals("dev"))
            this.passwordRecoveryGmail(event);
        else
            this.passwordRecoveryGmailWithHTML(event);

    }

    @SneakyThrows
    private void passwordRecoverySendGrid(OnPasswordRecoveryEvent event) {
        ...
        String subject = "POD-iSanté - Mettre à jour le mot de passe";
    ...
    private void passwordRecoveryGmail(OnPasswordRecoveryEvent event) {
        PasswordUpdateDto pass = event.getPasswordDto();
        String token = UUID.randomUUID().toString();
        service.createUpdatePasswordToken(pass, token);
        String recipientAddress = pass.getEmail();
    ...
    private void passwordRecoveryGmailWithHTML(OnPasswordRecoveryEvent event) throws MessagingException, UnsupportedEncodingException {
        PasswordUpdateDto pass = event.getPasswordDto();
        String token = UUID.randomUUID().toString();
        String recipientAddress = pass.getEmail();
        String subject = "POD-iSanté - Mettre à jour le mot de passe";
        ...
        String messageContent = "<!doctype html>\n" +
                ...
                "                      <td>\n" +
                "                        <p>Bonjour,</p>\n" +
                "                        <p>Pour mettre à jour votre mot de passe veuillez cliquez ci-dessous:</p>\n" +
