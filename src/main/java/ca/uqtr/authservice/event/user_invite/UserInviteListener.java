...
public class UserInviteListener implements
    ...
    public void onApplicationEvent(OnUserInviteEvent event) {
        if (mailService.equals("dev"))
            this.userInviteGmail(event);
        else
            this.userInviteGmailWithHTML(event);

    }

    @SneakyThrows
    private void userInviteSendGrid(OnUserInviteEvent event) {
        ...
        Mail mail = new Mail();
        mail.setFrom(new Email("sadegh.moulaye.abdallah@uqtr.ca", "POD ISANTE"));
    ...
    private void userInviteGmail(OnUserInviteEvent event) {
        UserInviteDto invite = event.getUserInviteDto();
        String token = UUID.randomUUID().toString();
        service.createUserInviteToken(invite, token);
        String recipientAddress = invite.getEmail();
        String subject = "POD-iSanté - Invitation.";
    ...
    private void userInviteGmailWithHTML(OnUserInviteEvent event) throws MessagingException, UnsupportedEncodingException {
        UserInviteDto invite = event.getUserInviteDto();
        String token = UUID.randomUUID().toString();
        service.createUserInviteToken(invite, token);
        String recipientAddress = invite.getEmail();
        ...
        String messageContent = "<!doctype html>\n" +
                ...
                "                      <td>\n" +
                "                        <p>Bonjour,</p>\n" +
                "                        <p>Pour vous inscrire, veuillez cliquez ci-dessous:</p>\n" +
